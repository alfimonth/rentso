...
class Sewa extends CI_Controller
{
    ...
    {
        $data['booking'] = $this->ModelBooking->getBookingStatus('Sudah Dibayar')->result_array();
    ...
    public function editkembali($kode)
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['kode'] = $kode;
            $this->load->view('back-end/sewaeditkembali', $data);
        } else {
            $POST = $this->input->post(null, true);
            $kode = $POST['id'];
            $status = $POST['status'];
            $this->ModelSewa->updateStatus($status, $kode);
            $this->session->set_flashdata('sewa', "<script>Swal.fire({icon: 'success',title: 'Status pembayaran berhasil diubah', showConfirmButton: false,timer: 1500})</script>");
            redirect(base_url('/sewa/pengembalian/'));
        }
    }
